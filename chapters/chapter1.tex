...
\begin{chapterabstract}
	In this chapter, we first propose an introduction to the problem of generative modeling and some solutions to tackle this problem. We propose an overview of the Generative Adversarial Networks \citep{Goodfellow2014} framework, which is a method to train deep neural networks as generative models that is particularly adapted to the task of image generation. We introduce some of its theoretical interpretations, as well as some of its variations and applications. We examine the problem of evaluating generative models, notably the intrinsic quality of generated samples, and review the most commonly used metrics and their limitations. We discuss the different limitations of the Generative Adversarial Networks approach and expose some goals among the different works, namely: enhancing the visual quality of the generated samples; maintaining their diversity; and conditioning the model.  Finally, we discuss the recent advances that have been made to overcome some of these limitations through neural networks architecture or objective variants.
\end{chapterabstract}
...
\end{minipage}\\
%
An simple example of generative model are Gaussian Mixture Models (\ac{GMM}) . They consist in a sum of $k$ Gaussian distributions $\mathcal{N}(\mu_i, \sigma_i^2), 1 \leq i \leq k$ which are all attributed a selection probability $\p{Z}(z=i) = \pi_k$, with $\vz \in \setZ$, so that $\p{\vx|\vz=k} = \mathcal{N}(\mu_k, \sigma_k^2)$ . The model is then formulated as 
%
\begin{equation}
	\pt{X}(\vx) = \sum_{\vz}\p{Z}(\vz)\pt{\vx|\vz}(\vx|\vz)\enspace,
\end{equation}
%
with log-likelihood 
%
\begin{equation}
	\log\sum_{\vx\sim\p{X}}\pt{X}(\vx)  = \sum_{\vx\sim\p{X}}\log\sum_{k=1}^k \pi_k \mathcal{N}(\vx|\mu_k, \sigma_k^2)\enspace.
\end{equation}
%
...
\begin{figure}
	...
	\caption[Latent variable model]{A mapping between a latent space $\setZ$ and the high-dimensional space of a dataset $\setX$.}
\end{figure}
...
While the marginal distribution $\pg{X}$ cannot be explicitly computed for any function $\G$, several solutions exist to overcome this problem and train deep generative models with latent variables anyways.  
...
\textbf{Pix2Pix} \citep{Isola2016} implemented this approach  explicitly by using paired samples $(\vx, \vy) \sim \p{\vx|\vy}$ forcing the generator to minimize the $\Lone$ reconstruction term between $\vx$ and $\G(\vy,\vz)$
...
\begin{algorithm}
	\begin{algorithmic}[]
		...
		\STATE update $\Dx$ by stochastic gradient descent of $ \sum_{i=1}^{m}\big( \lgan(\G_{YX}, \D_{X})\big)$\;
		\STATE update $\Dy$ by stochastic gradient descent of $ \sum_{i=1}^{m}\big(\lgan(\G_{XY}, D_{Y})\big)$\;
		\STATE sample a mini-batch $\lbrace \vx_i \rbrace_{i=1}^m$ from $\trainsetX$\;
		\STATE sample a mini-batch $\lbrace \vy_i \rbrace_{i=1}^m$ from $\trainsetY$\;
		\STATE update $\Gxy$ by stochastic gradient descent of
		\STATE \ \ \ \ $ \sum_{i=1}^n \big(\lgan(\G_{YX}, \D_{X})\big)  + \lambda \big(||\vx_i - \Gyx(\Gxy(\vx_i))||_1 +||\vy_i -\Gxy(\Gyx(\vy_i))||_1\big)$\;
		\STATE update $\Gyx$ by stochastic gradient descent of
		\STATE \ \ \ \ $ \sum_{i=1}^n \big(\lgan(\G_{XY}, D_{Y})\big)+ \lambda\big (||\vx_i - \Gyx(\Gxy(\vx_i))||_1 + ||\vy_i - \Gxy(\Gyx(\vy_i))||_1\big)$\;
		\UNTIL a stopping condition is met
	\end{algorithmic}
...
\section{Improving the Generative Adversarial Networks}
...
Although this distance can show nice properties, allowing for two-sample testing, it relies on the choice of the kernel $k$. Thus by using a fixed kernel, \ac{MMD} was used to formulate the different\textbf{ \ac{MMD}GAN} \citep{Li2017a,Dziugaite2015, Binkowski2018} approaches, which train GANs by estimating the \ac{MMD} with either gaussian or quadratic kernels. 
...
\textbf{Progressive GAN}, introduced by \citet{Karras2017}, allowed for the first high-dimensional 
