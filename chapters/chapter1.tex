...
%
\begin{equation}
	L_{VAE}(\F, \G) = \esp{\vx \sim \mathcal{N}(\mu_\vx, \Sigma_\vx)}\big[ ||\vx - \G(\vz)||^2_2 \big] - \DKL{\mathcal{N}(\mu_\vx, \Sigma_\vx)}{\p{Z}}\enspace.
\end{equation}
%
...
\begin{figure}
	...
	\caption[RealNVP affine transformations]{The affine transformation used in RealNVP. Left: Forward pass, Right: backward pass. A variable $\vx$ is split in two variables $\vx_1$ and $\vx_2$. The non-linear functions $s(\vx_1)$ and $t(\vx_1)$ are then used to compute the output variables $\vy_1 = \vx_1$ and $\vy_2 =  s(\vx_1) \vx_2 + t(\vx_1)$. Inverting this transformation can then be done by computing $\vx_1 = \vy_1$ and $\vx_2 =  (\vy_2  - t(\vy_1)) / s(\vy_1)$.  Figure by \citet{Dinh2017}}
	\label{fig:realnvp}
\end{figure}
...
\begin{equation}
	\label{eq:disc_cost}
	L_\D(\D, \G) =  \esp{\vx\sim \p{X}} [\log \D(\vx)] +  \esp{\Hat{\vx}\sim\pg{\vx}} [\log \D(1 - \Hat{\vx})] \enspace .
\end{equation}
%
...
\begin{equation}
		\max_\D L_\D(\D,\G) =  \esp{\vx\sim \p{X}} \Big[\log \frac{\p{X}(\vx)}{\p{X}(\vx) + \pg{X}(\vx)}\Big] +   \esp{\vx\sim \pg{X}} \Big[\log  1 - \frac{\p{\vx}(\vx)}{\p{X}(\vx) + \pg{X}(\vx)}\Big] \nonumber \enspace.
\end{equation}
...
\arg\min_\G\min_G\lgan = \arg\min_\G\max_\D \esp{\vx\sim \p{X}} [\log \D(\vx)] +  \esp{\vz\sim\p{Z}} [ \log 1 - \D(\G(\vz))] \enspace.
\end{equation}
%
...
On the other hand, the \textbf{Deep Convolutional GAN (\ac{DCGAN})} \citep{Radford2015} approach replaced the discriminator by a  fully convolutional network \citep{Springenberg2015} with strided convolutions and introduced \glnf{deconv}{deconvolutional} (or transposed convolutional) layers in the generator. It also introduced dropout \citep{Srivastava2014} and \glnf{batchnorm}{Batch Normalization} \citep{Ioffe2015}, and used both \ac{ReLU} \citep{Nair2010} and Leaky \ac{ReLU} \citep{Maas2013} as activation functions. \ac{DCGAN} showed much better results than the original GAN and LAPGAN, while being more computationally efficient than the latter, and thus became a standard baseline for image generation.
...
This approach was extended  with tricks for mitigating its instability \citep{Salimans2016}, such as adding \glnf{batchnorm}{Batch Normalization} \citep{Ioffe2015}, smoothing the 0/1 label used for training the discriminator or adding noise to the discriminator's input \citep{Sonderby2017}. They effectively helped stabilizing the training process. However, the \ac{DCGAN} approach remains limited in both the visual quality of the generated samples and in its ability to generate high-dimension images.
...
\begin{figure}[t]
	...
	\caption[ALI/BiGAN approaches]{Adversarially Learned Inference / Bidirectional GAN}
	\label{fig:ali}
\end{figure}
...
Although we restrict ourselves to image generation, it is worth noting that the application range of \ac{GAN}s covers text \citep{Guo2018}, video \citep{Clark2020} or sound \citep{Engel2018} generation. These application domains are beyond the scope of the thesis.
